<?php
class Grafikon_GdEditor implements Grafikon_EditorInterface {

    /**
     * Blend two images together with the first image as the base and the second image on top. Supports several blend modes.
     *
     * @param Grafikon_GdImage $image1 The base image.
     * @param Grafikon_GdImage $image2 The image placed on top of the base image.
    ...
    public function blend(&$image1, $image2, $type='normal', $opacity = 1.0, $position = 'top-left', $offsetX = 0, $offsetY = 0 ){
        ...
        $position = new Grafikon_Position($position, $offsetX, $offsetY);
    ...
    public function compare($image1, $image2) {
        ...
        if (is_string($image1)) { // If string passed, turn it into a Image object
            $image1 = Grafikon_GdImage::createFromFile($image1);
            $this->flatten( $image1 );
        }

        if (is_string($image2)) { // If string passed, turn it into a Image object
            $image2 = Grafikon_GdImage::createFromFile($image2);
            $this->flatten( $image2 );
        }

        $hash = new Grafikon_GdDifferenceHash();
    ...
    public function rotate(&$image, $angle, $color = null)
    {
        ...
        $color = ($color !== null) ? $color : new Grafikon_Color('#000000');
        list($r, $g, $b, $alpha) = $color->getRgba();
        ...
        imagedestroy( $old ); // Free resource
        $image = new Grafikon_GdImage( $new, $image->getImageFile(), $image->getWidth(), $image->getHeight(), $image->getType() );
    ...
    public function save($image, $file, $type = null, $quality = null, $interlace = false, $permission = 0755)
    {
        ...
        if (null === $type) {
            ...
            $type = Grafikon_ImageType::fromFileName($file); // Null given, guess type from file extension
            if (Grafikon_ImageType::UNKNOWN === $type) {
        ...
        switch (strtoupper($type)) {
            case Grafikon_ImageType::GIF :
                if($image->isAnimated()){
                    $blocks = $image->getBlocks();
                    $gift = new Grafikon_GifHelper();
                    $hex = $gift->encode($blocks);
                    if( false === file_put_contents($file, pack('H*', $hex)) ){
                        return false;
                    }
                    return true;
                } else {
                    return imagegif($image->getCore(), $file);
                }

                break;

            case Grafikon_ImageType::PNG :
                // PNG is lossless and does not need compression. Although GD allow values 0-9 (0 = no compression), we leave it alone.
                return imagepng($image->getCore(), $file);
                break;
            ...
            default: // Defaults to jpeg
                ...
                return imagejpeg($image->getCore(), $file, $quality);
        }
    }
     ...
     * @param Grafikon_Color $color The Color object. Default text color is black.
    ...
    public function text(&$image, $text, $size = 12, $x = 0, $y = 0, $color = null, $font = '', $angle = 0)
    {
        ...
        $color = ($color !== null) ? $color : new Grafikon_Color('#000000');
        $font  = ($font !== '') ? $font : Grafikon::fontsDir() . DIRECTORY_SEPARATOR . 'LiberationSans-Regular.ttf';
    ...
    private function _resize(&$image, $newWidth, $newHeight, $targetX = 0, $targetY = 0, $srcX = 0, $srcY = 0) {
        ...
        if ($image->isAnimated()) { // Animated GIF
            ...
            // Create blank image
            $newImage = Grafikon_GdImage::createBlank($newWidth, $newHeight);

            if (Grafikon_ImageType::PNG === $image->getType()) {
