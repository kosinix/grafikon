<?php
class Grafikon_ImagickEditor implements Grafikon_EditorInterface {
    
    /**
     * Blend two images together with the first image as the base and the second image on top. Supports several blend modes.
     *
     * @param Grafikon_ImagickImage $image1 The base image.
     * @param Grafikon_ImagickImage $image2 The image placed on top of the base image.
    ...
    public function compare($image1, $image2)
    {
        ...
        if (is_string($image1)) { // If string passed, turn it into a Image object
            $image1 = Grafikon_ImagickImage::createFromFile($image1);
            $this->flatten( $image1 );
        }

        if (is_string($image2)) { // If string passed, turn it into a Image object
            $image2 = Grafikon_ImagickImage::createFromFile($image2);
            $this->flatten( $image2 );
        }

        $hash = new Grafikon_ImagickDifferenceHash();
     ...
     *
     * @param Grafikon_ImagickImage $image
    ...
    public function crop(&$image, $cropWidth, $cropHeight, $position = 'center', $offsetX = 0, $offsetY = 0)
    {
        ...
        $image = new Grafikon_ImagickImage(
            $image->getCore(),
            $image->getImageFile(),
            $cropWidth,
            $cropHeight,
    ...
    public function save( $image, $file, $type = null, $quality = null, $interlace = false, $permission = 0755) {
        ...
        if (false === is_dir($targetDir)) { // Check if $file's directory exist
            // Create and set default permission
        ...
        switch (strtoupper($type)) {
            case Grafikon_ImageType::GIF :
                return $image->getCore()->writeImages($file, true); // Support animated image. Eg. GIF
            ...
            case Grafikon_ImageType::PNG :
                ...
                return $image->getCore()->writeImage($file);
            ...
            default: // Defaults to jpeg
                ...
                return $image->getCore()->writeImage($file); // Single frame image. Eg. JPEG
        }
    ...
    private function _resize(&$image, $newWidth, $newHeight) {
        ...
        if (Grafikon_ImageType::GIF == $image->getType()) { // Animated image. Eg. GIF
